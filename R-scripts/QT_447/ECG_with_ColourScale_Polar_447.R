...
ECG <- read_excel("C:/Users/alahmada/Desktop/CHI after review/Camera ready/R/QT_447.xlsx")
...
#next code for creating the colour vector
maximumColourVector <- 500 
colourVector <- NA
length(colourVector) <- 10000 #this equals to the length of ECG signal array  

for (R in 1:length(R_peaks_time))
{ if (R_peaks_time[R] >= 28)
...
write.csv(ECG_with_colourScale,"C:/Users/alahmada/Desktop/CHI after review/Camera ready/R/file2.csv")
...
half_RR_value1 <- (df_Rpeaks_RR[2] - df_Rpeaks_RR[1] ) / 2
#covert to millisconds 
half_RR_value1 <- half_RR_value1/1000
half_RR_value1

#caluclate the incresed value fo half RR0interval to draw the dashed lines
half_RR_value2 <- (df_Rpeaks_RR[4] - df_Rpeaks_RR[3] ) / 2
#covert to millisconds 
half_RR_value2 <- half_RR_value2/1000
half_RR_value2
...
p  +  coord_polar() +
  ...
  geom_vline(xintercept = (df_Rpeaks_RR[1]/1000) + half_RR_value1, linetype="dashed",  size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[2]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[3]/1000) + half_RR_value2,linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[4]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[5]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[6]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[7]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[8]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[9]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  geom_vline(xintercept = (df_Rpeaks_RR[10]/1000) + half_RR_value2, linetype="dashed", size=0.7) +
  
  scale_y_continuous(minor_breaks = seq(-1 , +1, 0.1),breaks = seq(-1, +1, 0.1), lim = c(-1, +1)) +
  scale_x_continuous(minor_breaks = seq(0 , 9.2, 0.04),breaks = seq(0, 9, 0.2), lim = c(0,9.2)) + 
  ...
  ggsave("C:/Users/alahmada/Desktop/CHI after review/Camera ready/R/QT_447_PC.png", width=15.61, height=12.93)
